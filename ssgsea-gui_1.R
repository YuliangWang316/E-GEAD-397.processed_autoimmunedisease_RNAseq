...
################################################################################################################
rm(list=ls())
script.dir <- "C:/Users/xjmik/Downloads/ssGSEA2.0-master" ## get folder the script is located in
...
while(!db.ok){
  ...
  if(os == 'Windows')
    gene.set.databases = choose.files(default = paste( script.dir, 'db/msigdb/msigdb.v7.4.symbols.gmt', sep='/' ), caption='Choose gene set database in gmt format. See Broad\'s MSigDB website for details.')
  else
